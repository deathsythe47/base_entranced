...
int PM_ItemUsable(playerState_t *ps, int forcedUse)
{
	...
	if (ps->duelInProgress || &g_entities[ps->clientNum] && g_entities[ps->clientNum].client->sess.siegeDuelInProgress)
	...
	case HI_SENTRY_GUN:
		...
		// remove old sentry
		if (!(level.clients[ps->clientNum].siegeClass != -1 &&
			bgSiegeClasses[level.clients[ps->clientNum].siegeClass].maxSentries > 0 &&
			level.sentriesDeployed[ps->clientNum] < bgSiegeClasses[level.clients[ps->clientNum].siegeClass].maxSentries)) {

			gentity_t *oldestSentry = NULL;
			int oldestTime = INT_MAX;

			for (int i = level.maxclients; i < MAX_GENTITIES; i++) {
				gentity_t *ent = &g_entities[i];

				if (ent->die == turret_die &&
					ent->genericValue3 >= 0 &&
					ent->genericValue3 < MAX_CLIENTS &&
					ent->genericValue3 == ps->clientNum) {

					if (ent->genericValue8 < oldestTime) {
						oldestTime = ent->genericValue8;
						oldestSentry = ent;
					}
				}
			}
			...
			if (oldestSentry) {
				G_FreeEntity(oldestSentry);
				--level.sentriesDeployed[ps->clientNum];
			}
...
static void PM_Weapon( void )
{
					...
					if (pm->ps->stats[STAT_HOLDABLE_ITEM])
					{
						...
						else
						{
							...
							if (bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag == HI_SHIELD && pm->ps->stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SHIELD)) {
								if (g_gametype.integer == GT_SIEGE && !iLikeToShieldSpam.integer && !G_ShieldSpamAllowed(pm->ps->clientNum) && (g_entities[pm->ps->clientNum].client->sess.sessionTeam == TEAM_RED || g_entities[pm->ps->clientNum].client->sess.sessionTeam == TEAM_BLUE) && !level.canShield[g_entities[pm->ps->clientNum].client->sess.sessionTeam]) {
	...
	else if (g_infiniteCharge.integer)
	{
		// check for item using
		if (pm->cmd.buttons & BUTTON_USE_HOLDABLE) {
			BG_ClearRocketLock(pm->ps); //test
	...
	if ((&g_entities[pm->ps->clientNum] && g_entities[pm->ps->clientNum].client->sess.siegeDuelInProgress && pm->ps->emplacedIndex) || (pm->ps->duelInProgress && pm->ps->emplacedIndex))
	{
		pm->ps->emplacedIndex = 0;
