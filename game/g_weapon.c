...
//---------------------------------------------------------
{
	...
	if (ent && ent->client && !g_friendlyFire.integer && g_gametype.integer == GT_SIEGE && level.wasRestarted)
...
{
	if (G_AntiSpam_BlockShot(ent, ANTISPAM_DOOR, WP_BOWCASTER, qtrue, SPAM_DISTANCE_BOWCASTER, forward))
	{
		return;
	}
...
//---------------------------------------------------------
{
	if (G_AntiSpam_BlockShot(ent, ANTISPAM_DOOR, WP_REPEATER, qtrue, SPAM_DISTANCE_BLOB, forward))
	{
		return;
	}
...
//------------------------------------------------------------------------------
{
	...
	if (self && self->client && !g_friendlyFire.integer && g_gametype.integer == GT_SIEGE && level.wasRestarted)
...
//---------------------------------------------------------
{
	if (G_AntiSpam_BlockShot(self, ANTISPAM_DOOR, WP_FLECHETTE, qtrue, SPAM_DISTANCE_GOLAN, forward)) // once for both projectiles
	{
		return;
	}
...
//---------------------------------------------------------
{
	if (G_AntiSpam_BlockShot(ent, ANTISPAM_DOOR, WP_ROCKET_LAUNCHER, altFire, SPAM_DISTANCE_ROCKET, forward))
	{
		return;
	}
...
{
	if (G_AntiSpam_ProjectileInHothLiftShaft(ent))
	{
		ent->think = G_FreeEntity;
		ent->nextthink = level.time;
		return;
...
//---------------------------------------------------------
{
	if (G_AntiSpam_BlockShot(ent, ANTISPAM_DOOR, WP_THERMAL, altFire, SPAM_DISTANCE_THERMAL, forward))
	{
		return 0;
	}
...
{ //create a laser trap entity
	...
	if (owner && owner->client && !g_friendlyFire.integer && g_gametype.integer == GT_SIEGE && level.wasRestarted)
...
void WP_PlaceLaserTrap( gentity_t *ent, qboolean alt_fire )
{
	...
	if (G_AntiSpam_BlockShot(ent, ANTISPAM_MINE, WP_TRIP_MINE, alt_fire, alt_fire ? SPAM_DISTANCE_MINES : SPAM_DISTANCE_PRIMARY_MINES, forward))
	{
		return;
	}
...
{//a fast rocket-like projectile
	...
	if (G_AntiSpam_BlockShot(ent, ANTISPAM_DOOR, WP_CONCUSSION, qfalse, SPAM_DISTANCE_CONC, forward))
	{
		return;
	}
			...
			for ( i = 0; i < MAX_VEHICLE_MUZZLES; i++ )
			{
				...
				if ( pVeh->m_iMuzzleTag[i] != -1 && pVeh->m_iMuzzleWait[i] < level.time )
				{
					...
					{//have enough ammo to shoot
						...
						if (missile) {
							if (ent && ent->lastPilot && ent->lastPilot->client && !g_friendlyFire.integer && g_gametype.integer == GT_SIEGE && level.wasRestarted)
